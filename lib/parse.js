...
export function parseResumeText(raw) {
  ...
  const data = {
    ...
    extra: [], education: [], work: [], projects: [], skills: [], skillTags: [], certs: [], awards: [], summary: '',
  };
  const notes = [];
  const used = new Set();
  ...
  const married = all.match(/(未婚|已婚|已婚已育|离异)/);
  if (married) data.extra.push({ k: '婚育', v: married[1] });
  ...
  if (!data.work.length) notes.push('没识别出工作经历，可能需要手动补');
